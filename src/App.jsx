...
const SAMPLE_CARDS = [
  ...
  // En la agencia de viajes - Im Reisebüro
  ...
  // El viaje. En automóvil - Die Reise. Im Wagen
...
function IconShuffle({ color=C.primaryDark, size=15 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none">
...
function Btn({ children, onClick, variant="primary", disabled, style={} }) {
  const base = {
    fontFamily: FONT, borderRadius: 8, fontWeight: 600, fontSize: 15,
    cursor: disabled ? "not-allowed" : "pointer", border: "none",
    minHeight: 44, display: "flex", alignItems: "center", justifyContent: "center",
    padding: "0 18px", opacity: disabled ? 0.45 : 1, transition: "opacity 0.15s",
    lineHeight: 1,
  };
  const variants = {
...
function StatCard({ value, label, color=C.primary }) {
  return (
    <div style={{ flex:1, minWidth:80, background:C.surface, borderRadius:8,
      padding:"12px 8px", border:`1px solid ${C.border}`, textAlign:"center", fontFamily:FONT }}>
...
// ── Modal shell — ensures consistent typography ───────────────
function Modal({ children }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.52)",
      display:"flex", alignItems:"center", justifyContent:"center",
      padding:16, zIndex:200, fontFamily:FONT }}>
...
export default function App() {
  const [screen,     setScreen]     = useState("deck");   // deck | study | done
  ...
  // Mark card as reviewed on first display
  useEffect(() => {
  ...
  const startStudy = useCallback((bs=batchSize, cat=filterCat, sh=shuffled) => {
    ...
    setSessionCount(q.length);
    setQueue(q); setIdx(0); setFlipped(false);
    setScreen("study"); setShowBatch(false);
    ...
    <Modal>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {BATCH_OPTIONS.map(({ n, label, note }) => {
          ...
          const total = n === 0 ? cards.length : Math.min(n, cards.length);
          return (
            <button key={n}
              onClick={() => { setBatchSize(n); startStudy(n, filterCat, shuffled); }}
              style={{ fontFamily:FONT, padding:"13px 16px", borderRadius:9,
      ...
      <button onClick={() => setShowBatch(false)}
        style={{ fontFamily:FONT, marginTop:14, width:"100%", padding:"11px 0",
      ...
      {[["german","Alemán *"],["spanish","Español"],["pronunciation","Pronunciación"],["notes","Notas"],["category","Categoría"]].map(([k,label]) => (
        <div key={k} style={{ marginBottom:12 }}>
          ...
          <input value={editCard[k]||""} onChange={e=>setEditCard(c=>({...c,[k]:e.target.value}))}
            style={{ fontFamily:FONT, width:"100%", padding:"11px 12px", borderRadius:8,
              border:`1.5px solid ${C.border}`, fontSize:15, boxSizing:"border-box",
  ...
  if (screen === "done") return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:FONT,
      ...
      <div style={{ background:C.surface, borderRadius:16, padding:32, maxWidth:360,
        width:"100%", textAlign:"center", border:`1px solid ${C.border}`,
        boxShadow:"0 2px 16px rgba(0,0,0,0.07)" }}>
        <div style={{ fontSize:40, marginBottom:12 }}>🎉</div>
        <p style={{ margin:"0 0 6px", fontSize:20, fontWeight:700, color:C.text }}>
          Sesión completada
        </p>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          <Btn onClick={() => startStudy(batchSize, filterCat, shuffled)} style={{width:"100%"}}>
            Repetir sesión
          </Btn>
          <Btn variant="outline" onClick={() => setShowBatch(true)} style={{width:"100%"}}>
            Nueva sesión
          </Btn>
          <Btn variant="ghost" onClick={() => setScreen("deck")} style={{width:"100%"}}>
            Volver al deck
          </Btn>
        </div>
      </div>
    ...
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:FONT, color:C.text }}>
      ...
      {toast && (
        <div style={{ position:"fixed", top:16, left:"50%", transform:"translateX(-50%)",
          ...
          whiteSpace:"nowrap", maxWidth:"90vw", fontFamily:FONT }}>
          {toast}
        </div>
      )}
        ...
        <div style={{ display:"flex", gap:6 }}>
          {[["deck","Deck"],["study","Estudiar"]].map(([s,label]) => (
            <button key={s}
              ...
              style={{ fontFamily:FONT, padding:"8px 14px", borderRadius:7, border:"none",
        ...
        <div style={{ maxWidth:640, margin:"0 auto", padding:"20px 16px" }}>
          ...
          <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10,
            ...
            <div>
              ...
              <p style={{ margin:"2px 0 0", fontSize:12, color:C.textMuted }}>
                {viewedToday > 0
                  ? `${viewedToday} vistas · ${pendingToday} pendientes`
            ...
            <div style={{ height:8, flex:1, maxWidth:140, background:C.border, borderRadius:4, overflow:"hidden" }}>
              <div style={{ height:"100%", background:C.primary, borderRadius:4,
                width: cards.length ? `${(viewedToday/cards.length)*100}%` : "0%",
            ...
            <div style={{ display:"flex", gap:7, flexWrap:"wrap", marginBottom:14 }}>
              {cats.map(cat => (
                <button key={cat} onClick={()=>setFilterCat(cat)}
                  style={{ fontFamily:FONT, padding:"6px 13px", borderRadius:16, minHeight:34,
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {cards.filter(c=>filterCat==="all"||c.category===filterCat).map(c => {
              const seenDate = c.lastReviewed
                ? new Date(c.lastReviewed).toLocaleDateString("es-AR",{day:"2-digit",month:"2-digit"}) : null;
              return (
                <div key={c.id} style={{ display:"flex", alignItems:"center",
                  padding:"11px 13px", background:C.surface, borderRadius:8,
                  ...
                  <div style={{ display:"flex", gap:6, flexShrink:0, alignItems:"center" }}>
                    ...
                    <button onClick={()=>setEditCard({...c})}
                      style={{ background:"none", border:"none", cursor:"pointer",
                        padding:"4px 6px", minHeight:36, display:"flex", alignItems:"center" }}>
                      <IconEdit/>
                    </button>
          ...
          {cards.length > 0 && (
            <button onClick={()=>setCards([])}
              style={{ fontFamily:FONT, marginTop:16, fontSize:12, color:C.danger,
        ...
        <div style={{ maxWidth:500, margin:"0 auto", padding:"20px 16px" }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:14 }}>
              {cats.map(cat => (
                <button key={cat}
                  ...
                  style={{ fontFamily:FONT, padding:"6px 12px", borderRadius:14, minHeight:34,
                    border:`1.5px solid ${filterCat===cat?C.primary:C.border}`,
                    background:filterCat===cat?C.primaryLight:"transparent",
                    color:filterCat===cat?C.primaryDark:C.textMuted,
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            ...
            <button onClick={toggleShuffle}
              style={{ fontFamily:FONT, display:"flex", alignItems:"center", gap:6,
          ...
          <div onClick={()=>setFlipped(f=>!f)}
            style={{ cursor:"pointer", perspective:1000, height:"min(280px,52vw)", minHeight:200 }}>
            <div style={{ position:"relative", width:"100%", height:"100%",
              ...
              <div style={{ position:"absolute", width:"100%", height:"100%",
                backfaceVisibility:"hidden", background:C.surface, borderRadius:14,
                ...
                {card?.category && (
                  <span style={{ position:"absolute", top:12, left:14, fontSize:11,
                    background:C.primaryLight, color:C.primaryDark, borderRadius:4,
                    padding:"2px 8px", fontWeight:600, fontFamily:FONT }}>
                    {card.category}
                  </span>
                )}
                <p style={{ fontFamily:FONT, fontSize:"clamp(20px,5vw,28px)", fontWeight:700,
                  textAlign:"center", color:C.text, lineHeight:1.3, margin:0 }}>
                  {card?.german}
                </p>
                <p style={{ fontFamily:FONT, fontSize:12, color:C.textLight, marginTop:14, marginBottom:0 }}>
              ...
              {/* Back */}
              <div style={{ position:"absolute", width:"100%", height:"100%",
                backfaceVisibility:"hidden", background:C.primaryLight, borderRadius:14,
                boxShadow:"0 2px 10px rgba(0,0,0,0.07)",
                display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
                padding:24, boxSizing:"border-box",
                transform:"rotateY(180deg)", border:`1.5px solid ${C.primary}`, gap:8 }}>
                <p style={{ fontFamily:FONT, fontSize:"clamp(18px,4.5vw,22px)", fontWeight:700,
                ...
                {card?.pronunciation && (
                  <p style={{ fontFamily:FONT, fontSize:14, color:C.textMuted, fontStyle:"italic", margin:0 }}>
                    /{card.pronunciation}/
                  </p>
                )}
                {card?.notes && (
                  <p style={{ fontFamily:FONT, fontSize:12, color:C.textMuted, textAlign:"center", margin:0 }}>
                    {card.notes}
                  </p>
                )}
              </div>
          ...
          <div style={{ display:"flex", gap:10, marginTop:16 }}>
            ...
            <Btn onClick={()=>nav(1)} style={{flex:2, minHeight:50}}>
              {idx+1 >= queue.length ? "Finalizar" : "Siguiente →"}
            </Btn>
          </div>
        </div>
      )}
